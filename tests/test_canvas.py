...
def test_draw_stroke_marks_canvas_dirty():
    canvas = VirtualCanvas(100, 100)
    canvas.apply_event({"type": "DRAW", "x": 10, "y": 10, "color": "#000000", "brushSize": 4, "startStroke": True})
    canvas.apply_event({"type": "DRAW", "x": 20, "y": 20, "color": "#000000", "brushSize": 4, "startStroke": False})
...
def test_line_event_draws_without_stroke_state():
    canvas = VirtualCanvas(100, 100)
    canvas.apply_event({"type": "LINE", "x": 0, "y": 0, "x2": 50, "y2": 50, "color": "#000000", "brushSize": 4})
    assert not canvas.is_blank()
...
def test_out_of_bounds_coordinates_are_clamped_not_crashing():
    canvas = VirtualCanvas(100, 100)
    canvas.apply_event({"type": "DRAW", "x": -50, "y": 9999, "color": "#000000", "brushSize": 4, "startStroke": True})
    canvas.apply_event({"type": "DRAW", "x": 5000, "y": -5000, "color": "#000000", "brushSize": 4, "startStroke": False})
    assert not canvas.is_blank()
